...
e gera um valor aleatório forte para `JWT_SECRET` (a variável de refresh token deixou de existir — ver `docs/ARCHITECTURE.md#autenticação`).
...
**A pasta `backend/prisma/migrations/` está vazia neste repositório** — as
migrations têm de ser geradas contra uma base de dados Postgres real com
rede disponível. Se estás a fazer deploy pela primeira vez (ex: Render +
Supabase) e apanhaste o erro `P3005: The database schema is not empty`,
segue **`docs/DEPLOY-SUPABASE-BASELINE.md`** — tem o procedimento completo.
